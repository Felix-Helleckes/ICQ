...
function init() {
  ...
  client.on('message', async (msg) => {
    let mediaData = null;
    if (msg.hasMedia && (msg.type === 'sticker' || msg.type === 'image')) {
      try {
...
async function getMessages(chatId) {
  ...
  const result = await Promise.all(msgs.map(async m => {
    let mediaData = null;
    if (m.hasMedia && (m.type === 'sticker' || m.type === 'image')) {
      try {
...
module.exports = { init, getQR, getStatus, getChats, getMessages, sendMessage };
